...
main() {
  ...
  test("requesting a lazy asset should cause it to be generated", () {
    initGraph(["app|foo.blub"], {
      "app": [[new LazyRewriteTransformer("blub", "blab")]]
    });
  ...
  test(
      "a lazy asset piped into a non-lazy transformer is eagerly compiled",
      () {
    var transformer = new LazyRewriteTransformer("blub", "blab");
    initGraph(["app|foo.blub"], {
      "app": [[transformer], [new RewriteTransformer("blab", "blib")]]
    });
    updateSources(["app|foo.blub"]);
    buildShouldSucceed();
    expect(transformer.numRuns, completion(equals(1)));
  ...
  test(
      "a lazy asset piped into a declaring transformer is compiled " "on-demand",
      () {
    initGraph(["app|foo.blub"], {
      "app": [
          [new LazyRewriteTransformer("blub", "blab")],
  ...
  test(
    ...
    initGraph(["app|foo.one"], {
      "app": [[transformer1], [transformer2], [transformer3], [transformer4]]
    });
  ...
  test(
    ...
    var transformer = new LazyRewriteTransformer("blub", "blab");
    initGraph(["app|foo.blub"], {
      "app": [[transformer], [new RewriteTransformer("txt", "out")]]
    });
    updateSources(["app|foo.blub"]);
    buildShouldSucceed();
    expect(transformer.numRuns, completion(equals(0)));
  ...
  test(
      "a lazy transformer followed by a non-lazy transformer is re-run " "eagerly",
      () {
    var rewrite = new LazyRewriteTransformer("one", "two");
    initGraph(["app|foo.one"], {
      "app": [[rewrite], [new RewriteTransformer("two", "three")]]
    });
  ...
  test(
    ...
    var rewrite = new LazyRewriteTransformer("one", "two");
    initGraph(["app|foo.one"], {
      "app": [[rewrite], [new DeclaringRewriteTransformer("two", "three")]]
    });
  ...
  test("a lazy asset works as a cross-package input", () {
    initGraph({
      ...
      "pkg1": [[new LazyRewriteTransformer("blub", "blab")],],
      "pkg2": [[new ManyToOneTransformer("txt")]]
    });
  ...
  test("a lazy transformer can consume secondary inputs lazily", () {
    initGraph({
      ...
      "app": [[new LazyManyToOneTransformer("txt")]]
    });
  ...
  test(
      "after being materialized a lazy transformer can be materialized again",
      () {
    var transformer = new LazyRewriteTransformer("blub", "blab");
    initGraph(["app|foo.blub"], {
      "app": [[transformer]]
  ...
  test(
    ...
    initGraph(["app|foo.txt"], {
      "app": [[new LazyBadTransformer("app|foo.out")]]
    });
  ...
  test("a lazy transformer passes through inputs it doesn't overwrite", () {
    initGraph(["app|foo.txt"], {
      "app": [[new LazyRewriteTransformer("txt", "out")]]
    });
  ...
  test("a lazy transformer doesn't pass through inputs it overwrites", () {
    initGraph(["app|foo.txt"], {
      "app": [[new LazyRewriteTransformer("txt", "txt")]]
    });
  ...
  test("a lazy transformer doesn't pass through inputs it consumes", () {
    initGraph(["app|foo.txt"], {
      "app": [[new LazyRewriteTransformer("txt", "out")..consumePrimary = true]]
    });
  ...
  test(
      "a lazy transformer that doesn't apply updates its passed-through asset",
      () {
    initGraph(["app|foo.txt"], {
      "app": [[new LazyRewriteTransformer("blub", "blab")]]
    });
