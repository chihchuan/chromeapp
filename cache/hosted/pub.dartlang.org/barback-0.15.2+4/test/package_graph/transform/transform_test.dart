...
main() {
  ...
  test("gets a transformed asset with the same path", () {
    initGraph(["app|foo.blub"], {
      "app": [[new RewriteTransformer("blub", "blub")]]
    });
    updateSources(["app|foo.blub"]);
  ...
  test("allows a transform to generate multiple outputs", () {
    initGraph({
      ...
      "app": [[new OneToManyTransformer("txt")]]
    });
  ...
  test(
      "doesn't get an output from a transform whose primary input is removed",
      () {
  ...
  test("handles an output moving from one transformer to another", () {
    ...
    initGraph({
      ...
      "app": [[new OneToManyTransformer("a"), new OneToManyTransformer("b")]]
    });
  ...
  test(
      "removes pipelined transforms when the root primary input is removed",
      () {
    initGraph(["app|foo.txt"], {
      "app": [
  ...
  test("gets an asset transformed by a sync transformer", () {
    initGraph(["app|foo.blub"], {
      "app": [[new SyncRewriteTransformer("blub", "blab")]]
    });
    updateSources(["app|foo.blub"]);
  ...
  group("Transform.hasInput", () {
    ...
    test("re-runs the transformer when an input stops existing", () {
      initGraph(["app|foo.txt", "app|bar.txt"], {
        'app': [[new HasInputTransformer(['app|bar.txt'])]]
      });
