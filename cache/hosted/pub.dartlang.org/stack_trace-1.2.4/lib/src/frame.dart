...
final _firefoxSafariFrame = new RegExp(
    r'^' r'(?:' // Member description. Not present in some Safari frames.
...
r'@' r')?' r'(.*?)' // The frame's URL.
r':' r'(\d*)' // The line number. Empty in Safari if it's unknown.
...
final _friendlyFrame =
    new RegExp(r'^(\S+)(?: (\d+)(?::(\d+))?)?\s+([^\d]\S*)$');
...
class Frame {
  ...
  factory Frame.parseVM(String frame) {
    ...
    var member = match[1].replaceAll(
        _asyncBody,
        "<async>").replaceAll("<anonymous closure>", "<fn>");
  ...
  factory Frame.parseV8(String frame) {
    ...
    if (match[2] != null) {
      ...
      return parseLocation(
          ...
          match[1].replaceAll(
              "<anonymous>",
              "<fn>").replaceAll("Anonymous function", "<fn>"));
    } else {
