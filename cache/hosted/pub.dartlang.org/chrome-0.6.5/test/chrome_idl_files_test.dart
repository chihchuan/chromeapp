...
void main() {
  group('ChromeIDLParser', () {
    ...
    Iterable<FileSystemEntity> chromeIdlFileEntities = new Directory(
        'idl').listSync(recursive: false, followLinks: false).where(idlFileExtTest);

    Iterable<FileSystemEntity> chromeIdlTestFileEntities = new Directory(
        'test/idl/chrome').listSync(
            recursive: false,
            followLinks: false).where(idlFileExtTest);
