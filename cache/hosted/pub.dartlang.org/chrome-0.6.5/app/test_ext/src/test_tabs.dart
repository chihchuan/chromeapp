...
void main() {
  group('chrome.tabs', () {
    ...
    setUp(() {
      ...
      return chrome.windows.create(createData).then((chrome.Window _window) => window =
          _window);
    });
    ...
    test('query -- one tab', () {
      ...
      chrome.tabs.query(
          queryInfo).then(expectAsync((List<chrome.Tab> foundTabs) {
    ...
    test('highlight', () {
      ...
      chrome.tabs.highlight(
          highlightInfo).then(expectAsync((chrome.Window newWindow) {
    ...
    test('update', () {
      ...
      chrome.tabs.update(
          updateProperties,
          window.tabs.first.id).then(expectAsync((chrome.Tab tab) {
        expect(tab.url, 'http://www.google.com/');
        expect(tab.active, isTrue);
        expect(tab.highlighted, isTrue);
        expect(tab.pinned, isTrue);
              ...
              (_tab) =>
                  newTab = _tab).then(
                      (_) =>
                          chrome.tabs.move(
                              [newTab.id],
                              moveProperties)).then(expectAsync((List<chrome.Tab> movedTabs) {
          ...
          createProperties).then(
              (_tab) =>
                  newTab1 = _tab).then(
                      (_) =>
                          chrome.tabs.create(
                              createProperties2)).then(
                                  (_tab) =>
                                      newTab2 = _tab).then(
                                          (_) =>
                                              chrome.tabs.move(
                                                  [newTab1.id, newTab2.id],
                                                  moveProperties)).then(expectAsync((List<chrome.Tab> movedTabs) {
        logMessage("movedTabs = ${movedTabs}");
        expect(movedTabs, isNotNull);
...
//              expect(movedTabs[1].index, anyOf(1, 2));
              ...
              (chrome.Tab tab) =>
                  chrome.tabs.remove(
                      [
                          tab.id])).then(
                              (_) =>
                                  chrome.tabs.query(queryInfo)).then(expectAsync((List<chrome.Tab> currentTabs) {
    ...
    test('detectLanguage', () {
      chrome.tabs.detectLanguage(
          window.tabs.first.id).then(expectAsync((String language) {
    ...
    test('onUpdated', () {
      ...
      chrome.tabs.update(
          updateProperties,
          window.tabs.first.id).then(expectAsync((_) {}));
    });
    ...
    test('onMoved', () {
      ...
      chrome.tabs.create(createProperties).then((chrome.Tab tab) {
        var subscription;
        subscription =
            chrome.tabs.onMoved.listen(expectAsync((chrome.TabsOnMovedEvent evt) {
    ...
    test('onActivated', () {
      ...
      chrome.tabs.create(createProperties).then((chrome.Tab tab) {
        var subscription;
        subscription =
            chrome.tabs.onActivated.listen(expectAsync((Map<dynamic, dynamic> evt) {
          expect(evt["tabId"], tab.id);
    ...
    test('onDetached', () {
      ...
      chrome.tabs.create(createProperties).then((chrome.Tab tab) {
        var subscription;
        subscription =
            chrome.tabs.onDetached.listen(expectAsync((chrome.OnDetachedEvent evt) {
        ...
        return chrome.windows.create(createData);
          ...
          (chrome.Window newWindow) =>
              chrome.windows.remove(newWindow.id)).then(expectAsync((_) {}));
    });
    ...
    test('onAttached', () {
      ...
      chrome.tabs.create(createProperties).then((chrome.Tab tab) {
        var subscription;
        subscription =
            chrome.tabs.onAttached.listen(expectAsync((chrome.OnAttachedEvent evt) {
    ...
    test('onRemoved', () {
      ...
      chrome.tabs.create(createProperties).then((chrome.Tab tab) {
        var subscription;
        subscription =
            chrome.tabs.onRemoved.listen(expectAsync((chrome.TabsOnRemovedEvent evt) {
          expect(evt.tabId, tab.id);
