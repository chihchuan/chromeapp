...
void main() {
  group('chrome.windows', () {
    ...
    setUp(() {
      ...
      return chrome.windows.create(windowCreateParams).then((_window) => window =
          _window);
    });
    ...
    test('get populate true', () {
      ...
      chrome.windows.get(
          window.id,
          getInfo).then(expectAsync((chrome.Window newWindow) {
    ...
    test('getAll populate true', () {
      ...
      chrome.windows.getAll(
          getInfo).then(expectAsync((List<chrome.Window> allWindows) {
        expect(allWindows.map((w) => w.id), contains(window.id));
        expect(
            allWindows.map((w) => w.tabs),
    ...
    test('create defaults', () {
      chrome.windows.create().then((chrome.Window window) {
        expect(window.tabs, hasLength(1));
    ...
    test('create incognito window', () {
      ...
      chrome.windows.create(createData).then((chrome.Window window) {
        expect(window.tabs, hasLength(1));
    ...
    test('update set size', () {
      chrome.WindowsUpdateParams updateInfo = new chrome.WindowsUpdateParams(
          left: 10,
          top: 10,
          width: 300,
          height: 300,
              ...
              id = window.id).then(
                  (_) =>
                      chrome.windows.remove(
                          id)).then((_) => chrome.windows.get(id)).catchError(expectAsync((_) {}));
    });
    ...
    test('onCreated', () {
      ...
      chrome.windows.create().then(
          (window) => chrome.windows.remove(window.id)).then(expectAsync((_) {}));
    });
    ...
    test('onFocusChanged', () {
      var subscription;
      subscription =
          chrome.windows.onFocusChanged.listen(expectAsync((windowId) {
