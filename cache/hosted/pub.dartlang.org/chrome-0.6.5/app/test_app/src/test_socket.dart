...
void main() {
  group('chrome.socket', () {
    test('create', () {
      socket.create(SocketType.TCP).then(expectAsync((CreateInfo createInfo) {
    ...
    test('connect', () {
      socket.create(SocketType.TCP).then(expectAsync((CreateInfo createInfo) {
        ...
        socket.connect(
            createInfo.socketId,
            "google.com",
            80).then(expectAsync((int connected) {
    ...
    test('read', () {
      socket.create(SocketType.TCP).then(expectAsync((CreateInfo createInfo) {
        ...
        socket.connect(
            createInfo.socketId,
            "128.138.140.44",
            13).then(expectAsync((int connected) {
          expect(connected, isZero);
          socket.read(
              createInfo.socketId).then(expectAsync((SocketReadInfo readInfo) {
            socket.disconnect(createInfo.socketId);
            socket.destroy(createInfo.socketId);
            expect(readInfo.resultCode, greaterThan(0));
            logMessage(
                "readInfo.data.runtimeType = ${readInfo.data.runtimeType}");
    ...
    test('write', () {
      socket.create(SocketType.TCP).then(expectAsync((CreateInfo createInfo) {
        ...
        socket.connect(
          ...
          expect(connected, isZero);
          var writeBuffer =
              new typed_data.Uint8List.fromList("GET /\n".codeUnits);
          socket.write(
              createInfo.socketId,
              new chrome.ArrayBuffer.fromBytes(
                  writeBuffer)).then(expectAsync((SocketWriteInfo writeInfo) {
            socket.disconnect(createInfo.socketId);
            socket.destroy(createInfo.socketId);
    ...
    test('getInfo', () {
      socket.create(SocketType.TCP).then(expectAsync((CreateInfo createInfo) {
        ...
        socket.connect(
          ...
          expect(connected, isZero);
          socket.getInfo(
              createInfo.socketId).then(expectAsync((SocketInfo socketInfo) {
